...
###   This script attempts to download the signature file SHA256SUMS.asc from btcr.org
...
RCSUBDIR="test/"
BASEDIR="https://btcr.org/bin/"
VERSIONPREFIX="btcr-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'btcr-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BTCr Core binary release signing key installed?"
   fi
