 ...
 *
 *   // network
 *   CBTCrNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.btcr.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CBTCrNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: BTCrNetwork
// //   -- signature requirements : Key1(User)
// class CBTCrNetworkVariable : public CGovernanceObject
// {
// private:
