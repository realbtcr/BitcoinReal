...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "BTCrNetwork") type = BTCrNetwork;
    if(strType == "BTCrNetworkVariable") type = BTCrNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == BTCrNetwork) s = "BTCrNetwork";
    if(type == BTCrNetworkVariable) s = "BTCrNetworkVariable";
