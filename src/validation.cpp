...
#if defined(NDEBUG)
# error "BTCr Core cannot be compiled without assertions."
#endif
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s BTCr additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    double dDiff;
    int64_t nSubsidy = 0;
    int64_t ProofOfLoyalty = 0;
    srand((unsigned)time(0));

    if (nPrevHeight == 2) { // No Premine on testnet
        return nSubsidy = 10000000000 * COIN;
    }

    // Ninja Launch, first 15'768'000 blocks 1 BTCr reward
    if (nPrevHeight <= 15768000) {
        nSubsidy = 0.1 * COIN; 
        ProofOfLoyalty = (rand()%5)/100 * COIN;
        return nSubsidy += ProofOfLoyalty *COIN;
    } else if (nPrevHeight <= 31536000) {
        nSubsidy = 0.5 * COIN;
        ProofOfLoyalty = (rand()%15)/100 * COIN;
        return nSubsidy += ProofOfLoyalty * COIN;
    } else  {
	nSubsidy = 1 * COIN;
    ProofOfLoyalty = (rand()%30)/100 * COIN;
    return nSubsidy += ProofOfLoyalty * COIN;   //Glaciation phase commences
	}
    // Commenting this out: BTCr is inflationary
    //if (nPrevHeight >= 10000000000000000) {
        //return 1;
    //}
    ...
    // Block reward starts at 20 and declines 50% every 2 years, getting in 10 years ~21M BTCr.
    ...
    // Minimum reward is 0.1
    if (nSubsidy < (0.1 * COIN)) {
        nSubsidy = 0.1 * COIN;
    }
    ...
    if (nPrevHeight <= 15768000) {
        ...
        typedef std::uniform_int_distribution<> E;
        E e(0, 0.15);
        return nSubsidy += e;
    } else {
        typedef std::uniform_int_distribution<> F;
        F f(0, 0.30);
        return nSubsidy += f;
    }
    */
    /*
    Proof Of Loyalty alternate
    if (nPrevHeight <= 15768000) {
        srand((unsigned)time(0)); 
        int64_t ProofOfLoyalty = 0; 
        ProofOfLoyalty = (rand()%5)/100;
        return nSubsidy += ProofOfLoyalty;
    } else if (nPrevHeight <= 31536000) {
        srand((unsigned)time(0)); 
        int64_t ProofOfLoyalty = 0; 
        ProofOfLoyalty = (rand()%15)/100;
        return nSubsidy += ProofOfLoyalty;
    } else {
        srand((unsigned)time(0)); 
        int64_t ProofOfLoyalty = 0; 
        ProofOfLoyalty = (rand()%30)/100;
        return nSubsidy += ProofOfLoyalty;    
    }
    */
...
{
    // MN gets 70% from block reward
    if (nHeight > 0 ) { //Masternodes activate block 0
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // BTCr : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(BTCr): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(BTCr): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // BTCr : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(BTCr): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(BTCr): spork is off, skipping transaction locking checks\n");
    }
