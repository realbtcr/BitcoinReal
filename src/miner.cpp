...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in btcr, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in BTCr
//
...
// ***TODO*** that part changed in btcr, we are using a mix with old one here for now
void static BTCrMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("BTCrMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("btcr-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("BTCrMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("BTCrMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BTCrMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("BTCrMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateBTCrs(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BTCrMiner, boost::cref(chainparams), boost::ref(connman)));
}
