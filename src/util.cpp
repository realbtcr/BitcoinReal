...
const char * const BTCr_CONF_FILENAME = "btcr.conf";
const char * const BTCr_PID_FILENAME = "btcrd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "btcr" is a composite category enabling all BTCr-related debug output
            if(ptrCategory->count(string("btcr"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BTCrCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BTCrCore
    // Mac: ~/Library/Application Support/BTCrCore
    // Unix: ~/.btcrcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTCrCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BTCrCore";
#else
    // Unix
    return pathRet / ".btcrcore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", BTCr_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty btcr.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override btcr.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", BTCr_PID_FILENAME));
