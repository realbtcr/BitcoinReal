...
const std::string BTCrGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString BTCrGUI::DEFAULT_WALLET = "~Default";

BTCrGUI::BTCrGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/btcr_logo_horizontal");
...
void BTCrGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BTCr address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and btcr: URIs)"));
    ...
    aboutAction = new QAction(tr("&About BTCr"), this);
    aboutAction->setStatusTip(tr("Show information about BTCr"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for BTCr"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BTCr addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BTCr addresses"));
    ...
    openAction->setStatusTip(tr("Open a btcr: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the BTCr help message to get a list with possible BTCr command-line options"));
...
void BTCrGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void BTCrGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool BTCrGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool BTCrGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void BTCrGUI::setWalletActionsEnabled(bool enabled)
{
...
void BTCrGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("BTCr client") + " " + networkStyle->getTitleAddText();
...
void BTCrGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void BTCrGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BTCrGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BTCrGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BTCrGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void BTCrGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void BTCrGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void BTCrGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BTCrGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BTCrGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BTCrGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void BTCrGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BTCr network", "", count));
    } else {
...
void BTCrGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BTCrGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void BTCrGUI::updateHeadersSyncProgressLabel()
{
...
void BTCrGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void BTCrGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void BTCrGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("BTCr"); // default title
...
void BTCrGUI::changeEvent(QEvent *e)
{
...
void BTCrGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BTCrGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BTCrGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BTCrUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void BTCrGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BTCrGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BTCrGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BTCrGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BTCrGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void BTCrGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void BTCrGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BTCrGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BTCrGUI::detectShutdown()
{
...
void BTCrGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void BTCrGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void BTCrGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BTCrGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void BTCrGUI::subscribeToCoreSignals()
{
...
void BTCrGUI::unsubscribeFromCoreSignals()
{
...
void BTCrGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void BTCrGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<BTCrUnits::Unit> units = BTCrUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const BTCrUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(BTCrUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(BTCrUnits::Unit u, BTCrUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(BTCrUnits::name(u)), this);
...
{
    setText(BTCrUnits::name(newUnits));
}
