    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("BTCr Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About BTCr Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  btcr-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_BTCr_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BTCrGUI::DEFAULT_UIPLATFORM));
        }
...
All the BTCr in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 BTCr, 0.1 BTCr, 1 BTCr and 10 BTCr -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("BTCr Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(BTCrGUI *window)
{
    if (!window)
        return nullptr;
